<template>
  <div
    :class="[
      `fixed top-0 bottom-0 left-0 right-0 bg-white z-[9999] flex justify-center items-center`,
      `${currentClass}`
    ]"
  >
    <div class="w-max flex flex-col gap-[10px]">
      <svg
        class="animate-spin h-10 w-10 text-green-700"
...
onMounted(() => {
  if (prop.hasInAnimate) currentClass.value = 'animate-load-in'
